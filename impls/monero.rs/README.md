...
release from https://github.com/MrCyjaneK/monero_c/releases.  The library can
be placed in any of the following locations:
- `../../release` (as in `monero_c/release`)
- `lib/` (as in `monero_c/impls/monero.rs/lib`)
- `target/debug` or `target/release` (alongside the built binary)

Its name should match your platform as in:
